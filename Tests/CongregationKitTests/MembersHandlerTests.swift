...
actor MembersHandlerTests {
    ...
    func testFetchAllMembersAllPages() async throws {
        ...
        var pageNumber = 1
        repeat {
            let response = try await handler.fetchAll(pageNumber: nil, pageSize: pageSize, nextPageToken: nextPageToken)
            if totalRecords == nil {
            ...
            if response.members.isEmpty || nextPageToken == nil { break }
            pageNumber += 1
